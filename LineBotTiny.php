...
class LINEBotTiny
{
    ...
    function exec_url_only($fullurl, $channelAccessToken)
    {
            $header = array(
                "Content-Type: application/json",
                "Content-Length: 0",
    ...
    {
        error_log($groupId);
        $this->exec_url_only('https://api.line.me/v2/bot/group/'.$groupId.'/leave', $this->channelAccessToken);
    }
    public function leaveRoom($roomId)
    {
        error_log($roomId);
        $this->exec_url_only('https://api.line.me/v2/bot/room/'.$roomId.'/leave', $this->channelAccessToken);
    }
    ...
    public function profil($userId)
    {
        ...
        return json_decode($this->exec_get('https://api.line.me/v2/bot/profile/'.$userId,$this->channelAccessToken), true);
       
    }
    public function getProfilFromGroup($userId, $groupId)
    {
        $url = 'https://api.line.me/v2/bot/group/'.$groupId.'/'.'member/'.$userId;
        error_log($url);
        return json_decode($this->exec_get($url,$this->channelAccessToken), true);
       
    }
    public function getProfilFromRoom($userId, $roomId)
    {
